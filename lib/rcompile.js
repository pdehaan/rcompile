// lib/rcompile.js - RCompile Module
//
...
function locateLibrary( moduleName ) {
    ...
    while ( i-- ) {
        path = this._baseDir + Path.sep +
            ...
            moduleName
        ;
        prefix = ( libraryList[i] ?
                libraryList[i] + PATH_SEP :
                ''
            ) +
            moduleName
        ;
...
function combine() {
    ...
    while ( ++i < moduleIds.length ) {
        ...
        moduleCode = FS.readFileSync(
            this._baseDir + Path.sep + moduleId, 'UTF-8'
        );
        code += 'require.set(\'' + moduleId +
